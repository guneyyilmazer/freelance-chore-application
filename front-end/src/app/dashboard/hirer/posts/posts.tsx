...
const Page = () => {
  ...
  const getPosts = async () => {
    const res = await fetch(
      `${BACKEND_SERVER_IP}/user/getPostsThisHirerShared`,
      {
        headers: {
  ...
  const completeContract = async (postId: string) => {
    const res = await fetch(`${BACKEND_SERVER_IP}/post/completeContract`, {
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${Cookies.get("Auth_Token")}`,
      },
    ...
    if (!response.error) {
      ...
      router.replace(`/post/leaveReview?id=${postId}`);
    }
  };
        ...
        <div className="w-[90%] flex flex-col">
          <div className="flex justify-between w-full my-20 items-center">
            <div className="flex flex-col">
              <div className="text-xl font-bold">Jobs You Have Created</div>
              <div className="flex">
          ...
          <div className="flex flex-wrap w-full justify-between">
            {posts.map((post: post,index:number) => (
              <div key={index} className="relative flex items-center">
                ...
                {post.user == user.userId && !post.completed ? (
                  <button
                    onClick={() => completeContract(post._id)}
                    className="absolute text-red-600 right-0"
                  >
                    End Contract
                  </button>
                ) : (
                  <div className="absolute right-0">Completed</div>
                )}
              </div>
            ))}
          </div>
